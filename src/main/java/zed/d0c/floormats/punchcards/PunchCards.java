    ...
    public void read(CompoundNBT nbt) {
        synchronized (Clusters.getClustersRegistry()) {
            ...
            for (String dimName : registryNBT.keySet()) {
                RegistryKey<World> dimType = RegistryKey.getOrCreateKey(Registry.WORLD_KEY, new ResourceLocation(dimName));
        ...
        synchronized (Clusters.getClustersRegistry()) {
            for (RegistryKey<World> dimType : Clusters.getClustersRegistry().keySet()) {
                if (dimType != null) {
                    ...
                    registryNBT.put(dimType.getLocation().toString(), clustersList);
                }
            }
        }
